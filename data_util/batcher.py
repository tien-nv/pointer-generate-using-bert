...
#my module
import data_util.bert_layer as bert_process
bert_pro = bert_process.BertData()
...
class Example(object):
  ...
  def __init__(self, article, abstract_sentences, vocab):
    ...
    self.enc_subtoken_ids, self.enc_segments_ids, self.enc_cls_ids, self.enc_sep_ids, self.enc_len_cls, self.enc_len_sep = bert_pro.preprocess(src_words)
    self.enc_len = len(self.enc_subtoken_ids)# store the length after truncation but before padding
  ...
  def pad_encoder_input(self, max_len, pad_id = bert_pro.pad_vid):
    # while len(self.enc_input) < max_len:
    #   self.enc_input.append(pad_id)
    # if config.pointer_gen:
    #   while len(self.enc_input_extend_vocab) < max_len:
    #     self.enc_input_extend_vocab.append(pad_id)
    while len(self.enc_subtoken_ids) < max_len:
      self.enc_subtoken_ids.append(pad_id)
      self.enc_segments_ids.append(pad_id)
    if config.pointer_gen:
      while len(self.enc_input_extend_vocab) < max_len:
        self.enc_input_extend_vocab.append(pad_id)

# self.enc_cls_ids, self.enc_sep_ids
  def pad_cls_sep_input(self, max_cls_len,max_sep_len, pad_id = -1):
    while len(self.enc_cls_ids) < max_cls_len:
      self.enc_cls_ids.append(pad_id)
    while len(self.enc_sep_ids) < max_sep_len:
      self.enc_sep_ids.append(pad_id)
...
class Batch(object):
  ...
  def init_encoder_seq(self, example_list):
    ...
    max_enc_len_cls = max([ex.enc_len_cls for ex in example_list])
    max_enc_len_sep = max([ex.enc_len_sep for ex in example_list])
    # # Pad the encoder input sequences up to the length of the longest sequence
    for ex in example_list:
      ex.pad_encoder_input(max_enc_seq_len)
      ex.pad_cls_sep_input(max_enc_len_cls,max_enc_len_sep)
    # Initialize the numpy arrays
    # Note: our enc_batch can have different length (second dimension) for each batch because we use dynamic_rnn for the encoder.
    self.enc_batch_ids = np.zeros((self.batch_size, max_enc_seq_len), dtype=np.long)
    self.enc_batch_segs = np.zeros((self.batch_size, max_enc_seq_len), dtype=np.long)
    self.enc_lens = np.zeros((self.batch_size), dtype=np.int32)
    self.enc_padding_mask = np.zeros((self.batch_size, max_enc_seq_len), dtype=np.float32)
    self.enc_cls_ids = np.zeros((self.batch_size, max_enc_len_cls), dtype=np.int32)
    self.enc_sep_ids = np.zeros((self.batch_size, max_enc_len_sep), dtype=np.int32)
    # Fill in the numpy arrays
    for i, ex in enumerate(example_list):
      self.enc_batch_ids[i, :] = ex.enc_subtoken_ids[:]
      self.enc_batch_segs[i, :] = ex.enc_segments_ids
      self.enc_lens[i] = ex.enc_len
      self.enc_cls_ids[i,:] = ex.enc_cls_ids[:]
      self.enc_sep_ids[i,:] = ex.enc_sep_ids[:]
      while len(self.enc_cls_ids[i]) < max_enc_len_cls:
        self.enc_cls_ids[i].append(-1)
      while len(self.enc_sep_ids[i]) < max_enc_len_sep:
        self.enc_sep_ids[i].append(-1)
